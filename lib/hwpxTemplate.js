...
// 원본 텍스트가 길어 여러 줄(hp:lineseg 여러 개)로 나뉘어 있던 문단의 텍스트를 짧게 바꾸면,
// 남은 줄 정보가 더 이상 존재하지 않는 글자 위치(textpos)를 가리키게 되어 한글이 파일을
// "손상되었거나 변조"된 것으로 판단한다. 줄 정보를 1개로 리셋해 한글이 다시 계산하게 한다.
function resetLineSegArray(tNode) {
  ...
  const segs = directChildren(linesegArray, "lineseg");
  for (let i = 1; i < segs.length; i++) {
    linesegArray.removeChild(segs[i]);
  }
  if (segs[0]) {
