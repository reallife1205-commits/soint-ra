...
// ---- 표3 토양오염실태조사 (물질쌍 가변) ----
// selectedSubstances: 챕터02 "측정항목 선택" 체크박스로 고른 물질만 표에 싣는다(전체 다 보여줄 필요 없음).
// 아직 저장된 선택값이 없는(과거) 케이스는 null이 오므로, 그때만 데이터가 있는 물질 전부를 보여주는 걸로 대체한다.
function fillSurvey(table, surveyRows, selectedSubstances) {
  ...
  const candidateKeys = selectedSubstances === null ? allKeys : allKeys.filter((k) => selectedSubstances.includes(k));
