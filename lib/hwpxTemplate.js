...
export async function replaceImage(zip, doc, picNode, imageBuffer, ext) {
  ...
  if (dims && dims.width > 0 && dims.height > 0) {
    ...
    // curSz/hp:sz(화면에 실제로 보이는 박스 크기)는 템플릿 칸 모양 그대로였는데, 그 칸이
    // 원래 다른 사건 사진의 가로세로 비율(예: 가로로 넓은 항공사진)에 맞춰져 있어서, 세로로
    // 찍은 세로사진(휴대폰 인물모드 등)을 끼우면 비율이 안 맞아 위아래가 잘려 보였다. 새
    // 이미지의 실제 비율대로 박스 안에 꽉 차지 않게(letterbox) 줄여서, 잘리는 대신 사진
    // 전체가 다 보이게 한다.
    // 1x1 빈 이미지(BLANK_PNG)는 실제 사진이 아니라 비율 맞출 의미가 없어서 원래 칸 모양 그대로 둔다.
    const curSz = firstDescendant(picNode, "curSz");
    const szEl = picNode.getElementsByTagNameNS(HP_NS, "sz")[0];
    if (curSz && dims.width > 1 && dims.height > 1) {
      const boxW = parseInt(curSz.getAttribute("width"), 10) || 0;
      const boxH = parseInt(curSz.getAttribute("height"), 10) || 0;
      if (boxW > 0 && boxH > 0) {
