...
// (지우지 않고) 빈 텍스트로만 비운다 — 문단을 지우면 뒤따르는 문단·표에 남아있는 캐시된 줄
// 위치 정보(hp:lineseg의 vertpos 등)가 실제 내용과 어긋나서 한글에서 글자가 겹쳐 보이는
// 문제가 있었음. 문단 개수를 템플릿 그대로 유지하면 그 어긋남이 훨씬 줄어든다.
...
// 1x1 투명 PNG — 다른 사건의 실제 사진(그림/표 이미지)을 대체할 케이스별 이미지가 아직 없어서,
// 최소한 그 사진이 보이지는 않게 비우는 용도.
const BLANK_PNG = Buffer.from(
  "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=",
  "base64"
);

// table 안의 사진을 전부 빈 이미지로 바꾼다. 2.1/2.2의 "그림" 자리, 그리고 현장사진 표(4장)가
// 실제로는 다른 사건의 실제 사진·회사명이 박혀있던 곳이라, 케이스별 대체 데이터가 없는 지금은
// 비워둔다. clearCaptions를 켜면 사진이 없는 셀(현장사진 표의 회사명 캡션 등)의 텍스트도 지운다
// — 현장사진 표는 안건마다 관련 회사 수가 달라 고정된 라벨로 자동 채울 수 없어(기존 개발자 주석
// 참고) 구조를 새로 만드는 대신 비워서 다른 사건 회사명만 제거한다.
async function blankTableImages(zip, table, { clearCaptions = false } = {}) {
  if (!table) return;
...
// 표로 감싸지 않은 채 본문 문단에 직접 박힌 그림(예: 2.1 "[표 1]" 이미지, 3.4 "배치도면")도
// 같은 이유로 비운다. headingText로 시작하는 문단을 찾아 그 안의 그림만 대상으로 한다.
async function blankParagraphImages(zip, doc, headingText) {
  const paragraphs = getSectionParagraphs(doc);
  const target = paragraphs.find((p) => getParagraphText(p).trim().startsWith(headingText));
  if (!target) return;
  for (const pic of getPics(target)) {
    await replaceImage(zip, null, pic, BLANK_PNG, "png");
  }
}
...
export async function buildReportHwpx(templateBuffer, payload) {
  ...
  // 사진·회사명 캡션만 비우고, 보고서 작성자가 한글에서 직접 채우게 둔다.
  await blankTableImages(zip, tables[TBL.FIELD_PHOTOS], { clearCaptions: true });

  // 2.1/2.2의 "그림"/"[표 1]" 자리에 박혀있던 다른 사건의 실제 사진을 비운다(케이스별 대체
  // 이미지가 아직 없음). 3.4 배치도면도 같은 문제라 함께 비운다.
  await blankTableImages(zip, tables[TBL.SAMPLE_POINT_IMAGE]);
  await blankTableImages(zip, tables[TBL.SURROUNDING_IMAGE]);
  await blankParagraphImages(zip, doc, "[표 1] 오염면적");
  await blankParagraphImages(zip, doc, "<그림> 배치도면");
