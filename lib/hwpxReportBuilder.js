...
function fillOverview(table, caseInfo, overview, contaminationRows) {
  ...
  setCellText(table, 0, 1, caseInfo.address || "-");
...
// ---- 표3 토양오염실태조사 (물질쌍 가변) ----
function fillSurvey(table, surveyRows) {
  const presentKeys = [...NETWORK_SUBSTANCE_KEYS_BLOCK1, ...NETWORK_SUBSTANCE_KEYS_BLOCK2].filter((k) =>
...
async function fillPhotoGrid(zip, table, images, labels) {
  ...
  for (const row of rows) {
    for (const cell of getCells(row)) {
      ...
      if (labels) {
        setTNodeTextOnCell(cell, labels[labelIdx] || "");
        labelIdx++;
      }
    }
  }
...
function setTNodeTextOnCell(cell, text) {
  const tNodes = Array.from(cell.getElementsByTagNameNS("http://www.hancom.co.kr/hwpml/2011/paragraph", "t"));
...
export async function buildReportHwpx(templateBuffer, payload) {
  const { caseInfo, overview, contaminationRows, networkRows, surveyRows, ownership, factoryHistoryItems, judgments, legalSummary, costCapacityItems, aerialImages, fieldPhotoImages } =
    payload;
  ...
  fillSurvey(tables[TBL.SURVEY], surveyRows);
  ...
  await fillPhotoGrid(zip, tables[TBL.FIELD_PHOTOS], fieldPhotoImages, FIELD_PHOTO_LABELS);
