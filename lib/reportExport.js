...
// ---- 과학적 기법에 의한 오염원인 추정 (내용은 추후 작성 — 양식만 제공) ----

function buildScientificMethodSection() {
  return [heading("5. 과학적 기법에 의한 오염원인 추정", HeadingLevel.HEADING_1), noteBody("※ 추후 작성 예정")];
}
...
function buildOverviewSection(caseInfo, overview, contaminationRows) {
  ...
  const rangeText =
    totals.area || totals.volume || totals.minStart !== null
      ? `면적 : ${formatSum(totals.area)} m², 부피 : ${formatSum(totals.volume)} m³, 심도 : ${
  ...
  const infoRows = [
    ...
    ["토양오염물질", substances.length ? substances.join(", ") : "-"],
    ["오염물질농도", totals.maxConc !== null ? `${formatSum(totals.maxConc)} mg/kg (최고농도)` : "-"],
    ["정화범위", rangeText],
    [
      "정밀조사 시기",
...
export async function buildReportDocx(payload) {
  ...
  const children = [
    ...
    ...buildOverviewSection(caseInfo, overview, contaminationRows),
