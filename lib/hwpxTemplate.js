...
export async function replaceImage(zip, doc, picNode, imageBuffer, ext) {
  ...
  if (dims && dims.width > 0 && dims.height > 0) {
    ...
    // 화면에 실제로 쓰이는 표시 크기는 hp:curSz가 아니라 hp:sz다(항공사진 칸 일부는 curSz가
    // 0x0인데도 정상 크기로 표시됨 — curSz는 일부 칸에서 비어있는 값이라 기준으로 삼으면 안
    // 됨). 다른 도형과 묶여(group) 있던 사진은 hp:sz가 자신이 아니라 부모 묶음 컨테이너에
    // 있으니 그쪽도 확인한다. hp:sz 기준으로 새 사진의 실제 비율이 원래 칸 안에 다
    // 들어오도록(letterbox) 줄이고, curSz도 같은 값으로 맞춰 둘이 어긋나지 않게 한다.
    let szEl = picNode.getElementsByTagNameNS(HP_NS, "sz")[0];
    if (!szEl && picNode.parentNode) {
      szEl = Array.from(picNode.parentNode.childNodes).find(
        (n) => n.nodeType === 1 && n.localName === "sz"
      );
    }
    const curSz = firstDescendant(picNode, "curSz");
    if (szEl && dims.width > 1 && dims.height > 1) {
      const boxW = parseInt(szEl.getAttribute("width"), 10) || 0;
      const boxH = parseInt(szEl.getAttribute("height"), 10) || 0;
      if (boxW > 0 && boxH > 0) {
        const scale = Math.min(boxW / w, boxH / h);
        const fitW = Math.max(1, Math.round(w * scale));
        const fitH = Math.max(1, Math.round(h * scale));
        szEl.setAttribute("width", String(fitW));
        szEl.setAttribute("height", String(fitH));
        if (curSz) {
          curSz.setAttribute("width", String(fitW));
          curSz.setAttribute("height", String(fitH));
        }
      }
    }
  }
