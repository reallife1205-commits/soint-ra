...
// 체크박스처럼 hp:rect가 텍스트박스(hp:drawText)를 담는 용도로 쓰인 경우는 지우면 안 되니,
// drawText가 없는(순수 장식용) rect만 골라서 지운다.
function removeStrayOverlayShapes(doc) {
  const rects = Array.from(doc.getElementsByTagNameNS(HP_NS, "rect"));
  rects.forEach((r) => {
    const hasDrawText = r.getElementsByTagNameNS(HP_NS, "drawText").length > 0;
    if (!hasDrawText && r.parentNode) r.parentNode.removeChild(r);
  });
}
...
export async function buildReportHwpx(templateBuffer, payload) {
  ...
  // "[표 1] 오염면적 및 오염범위" 캡션이 붙은 그림은 2.1의 "그림 시료채취지점"과는 별개의
  // 독립된 이미지라, 따로 채우거나(없으면) 비워야 함 — 예전엔 같은 것으로 착각해서 빠뜨렸음.
  await fillOrBlankParagraphImage(zip, doc, "[표 1] 오염면적", pollutionMapImages);
