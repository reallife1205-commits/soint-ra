...
function fillSummaryTable(table, judgments, legalSummary) {
  const rows = [
    {
      content: checklistSummaryLine(judgments.ownership_lease, [
  ...
  rows.forEach((r, i) => {
    ...
    // 검토내용 칸도 검토의견 칸과 같은 이유(다른 사건 템플릿의 번호목록용 내어쓰기 문단속성)로
    // 줄바꿈되면 체크박스가 맨 앞이 아니라 오른쪽으로 밀린 위치에서 시작해 보였다 — 같이 맞춘다.
    resetParaIndent(cells[1]);
